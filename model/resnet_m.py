...
def create(x, num_outputs):
    ...
    with tf.variable_scope('ResNet-Modified', reuse=tf.AUTO_REUSE):
        ...
        resBlock = seNet(resBlock,filters=4*64,name='SENET_1')
        ...
        resBlock = seNet(resBlock,filters=4*128,name='SENET_2')
        ...
        resBlock = seNet(resBlock,filters=4*256,name='SENET_3')
        ...
        result = tf.layers.dense(output,units=num_outputs,kernel_initializer=initializer,name='FC')

        return result

    pass

def ResBlock(input,channels,stride,name='',initializer=tf.contrib.layers.variance_scaling_initializer(),regularizer=keras.regularizers.l2(),is_training=False):
    blocklist = []
...
def seNet(input,filters,ratio=SQUEEZE_RATIO,initializer=tf.contrib.layers.variance_scaling_initializer(),name=''):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        squeeze = tf.reduce_mean(input,axis=[1,2],keep_dims=False,name='Global_Average_Pooling')
        # squeeze = tf.reshape(squeeze, [-1,filters])
        excitation = tf.layers.dense(squeeze,units=filters*ratio,kernel_initializer=initializer,name='SE_FC1')
        excitation = tf.nn.relu(excitation,name='SE_ReLU')
        excitation = tf.layers.dense(excitation,units=filters,kernel_initializer=initializer,name='SE_FC2')
